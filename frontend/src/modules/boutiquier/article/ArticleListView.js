...
export class ArticleView extends AbstractView {
  constructor(app, { params, route } = {}) {
    ...
    this.localArticles = [];
    this.formModal = new ArticleFormModal(app);
  }
  ...
  async setup() {
    try {
      ...
      console.log(error);
      this.showError("Erreur de chargement des articles");
    }
  }
    ...
    header.innerHTML = `
      <div>
        ...
        <p class="text-sm text-gray-600">
          ${this.localArticles.length} articles
        </p>
      </div>
    `;
      ...
      columns: [
        {
          ...
          sortable: true,
          render: (item) => {
            return `<div class="font-medium">${item.titre}</div>`;
          },
        },
        {
          header: "Contenu",
          key: "contenu",
          render: (item) => {
            return `<div class="text-sm line-clamp-2">${item.contenu}</div>`;
          },
        },
        {
          ...
          sortable: true,
          render: (item) => {
            return new Date(item.date_publication).toLocaleDateString();
          },
        },
        {
          header: "Boutiquier",
          key: "id_boutiquier",
          render: (item) => {
            return `Boutique #${item.id_boutiquier}`;
          },
        },
        {
          header: "Statut",
